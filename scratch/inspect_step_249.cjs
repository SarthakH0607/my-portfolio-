...
if (fs.existsSync(log2)) {
  const content = fs.readFileSync(log2, 'utf8');
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (data.step_index === 249) {
        const text = data.content || data.output || '';
        if (text && text.includes('VisionTower.jsx')) {
          const cleaned = cleanCode(text);
          fs.writeFileSync(`c:\\Users\\User\\Desktop\\Portfolio\\scratch\\vision_249.txt`, cleaned, 'utf8');
          console.log(`Wrote step 249 (log2) to scratch/vision_249.txt`);
        }
      }
    } catch (e) {}
  }
} else {
  console.log("log2 not found");
}
