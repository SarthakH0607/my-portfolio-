...
def main():
    ...
    steps = [2294, 108, 394, 630, 1771, 229, 17]
    for step in steps:
            ...
            for line in f:
                ...
                try:
                    ...
                    if data.get('step_index') == step:
                        ...
                        if 'Navbar' in content and 'export default' in content:
                            cleaned = clean_code(content)
                            if cleaned and 'Walkthrough' not in cleaned and len(cleaned) > 1500:
                                print(f"SUCCESS: Step {step} has actual Navbar code, len={len(cleaned)}")
                                out_path = r'C:\Users\User\Desktop\Portfolio\src\components\Navbar.jsx'
